...
async function main() {
    ...
    // By default JOIN_FEE is 0, but RAID_FEE is 5000 (0.005 USDC)
    ...
    await usdc.connect(user1).approve(potAddress, ethers.MaxUint256); // Pot moves money too? No, Core moves it to Pot.
    // Core calls pot.depositFrom(msg.sender), which calls usdc.transferFrom(msg.sender, pot)
    // So user needs to approve POT? Let's check Pot code. 
    // Usually logic contract calls transferFrom. 
    // CartelCore: `pot.depositFrom(msg.sender, FEE)`
    // CartelPot: `function depositFrom(from, amount) { usdc.transferFrom(from, address(this), amount); }`
    // So user must approve CARTEL_POT to spend their USDC. 
    ...
    // Verify Shares
    const bal1 = await core.sharesContract().then(a => shares.balanceOf(user1.address, 1));
    ...
    // User1 raids User2
    // Raid fee: 5000 (0.005 USDC)
    // Expected Result: User1 pays 5000 USDC. Pot balance increases.
    // 10% steal chance (wait, logic is deterministic steal amount? No, chance based? 
    // Default NORMAL_RAID_STEAL_BPS = 10% (1000). Always succeeds?
    // Code: `if (stolen > 0) { ... success = true }`
    // Yes, simple logic for MVP.
    ...
    else console.log("⚠️ Raid Share Transfer unexpected values (might differ based on contract calculation).");

    // --- TEST 3: HIGH STAKES RAID ---
    console.log("🧪 Test 3: High Stakes Raid...");
    // High Stakes Fee: 15000 (0.015 USDC)
    // Steal 20%
    await core.connect(user2).highStakesRaid(user1.address); // Revenge!
    ...
    // User1 had 110. 20% of 110 = 22.
    // User1 -> 88. User2 -> 90 + 22 = 112.
    // Self Penalty? 3% of User2 (90 * 0.03 = 2.7 -> 2).
    // User2 -> 112 - 2 = 110.

    // --- TEST 4: CLAIM PROFIT ---
    console.log("🧪 Test 4: Claim Profit...");
    // Distribute Revenue
    // Revenue Pool should have fees: Join(0) + Raid(5000) + HighStakes(15000) = 20000.
    // Distribute requires 24h passed. We need to cheat time.
