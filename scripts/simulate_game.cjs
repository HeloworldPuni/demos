...
async function main() {
    console.log("🚀 Starting Full Simulation Script (Standalone)...\n");
    ...
    const pot = await CartelPot.deploy(await usdc.getAddress());
    await pot.waitForDeployment();

    const CartelCore = await ethers.getContractFactory("CartelCore");
    const core = await CartelCore.deploy(await shares.getAddress(), await pot.getAddress(), await usdc.getAddress());
    await core.waitForDeployment();

    const AgentVault = await ethers.getContractFactory("AgentVault");
    const agent = await AgentVault.deploy(await usdc.getAddress(), await core.getAddress());
    ...
    // Usually strict equality check is safer for BigInt unless Chai plugins loaded
    if (pending1 <= 0n) throw new Error("Pending profit should be > 0");
    ...
    expect(usdcAfterClaim).to.not.equal(usdcBeforeClaim);
