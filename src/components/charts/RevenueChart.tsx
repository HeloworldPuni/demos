...
export default function RevenueChart({ range = '7d' }: { range?: string }) {
    ...
    return (
        <div className="w-full h-32 mt-4 flex items-end justify-between gap-1 select-none">
            {data.map((d, i) => {
                const heightPct = Math.max(((d.revenue || 0) / maxRevenue) * 100, 4); // Min 4% height for visibility
                ...
                return (
                    <div key={i} className="group relative flex-1 flex flex-col justify-end items-center h-full">
                        ...
                        {/* Bar */}
                        <div
                            className="w-full max-w-[20px] bg-[#4FF0E6]/20 group-hover:bg-[#4FF0E6] transition-colors rounded-t-sm"
                            style={{ height: `${heightPct}%` }}
                        />

                        {/* X Axis Label (only if space permits/sparsely) */}
                        <div className="mt-1 text-[8px] text-zinc-600 font-mono hidden sm:block">
                            {/* Show label for first, last, and every 3rd */}
                            {(i === 0 || i === data.length - 1 || i % 3 === 0) ? dateLabel : ''}
                        </div>
                    </div>
                );
            })}
